 ...
 * 社团财务审批：拒绝
 */
Finance.refuse = function () {
    if (this.check()) {
        var operation = function () {
            var financeId = Finance.seItem.id;
            var ajax = new $ax(Feng.ctxPath + "/finance/apply_refuse", function (data) {
                Feng.success("已拒绝财务申请");
                Finance.table.refresh();
            }, function (data) {
        ...
        Feng.confirm("是否拒绝此社团财务申请? ", operation);
    }
};
...
Finance.agree = function () {
    if (this.check()) {
        var operation = function () {
            var financeId = Finance.seItem.id;
            var ajax = new $ax(Feng.ctxPath + "/finance/apply_agree", function (data) {
                Feng.success("审批成功!");
                Finance.table.refresh();
            }, function (data) {
                Feng.error("审批失败!请联系网信部!" + data.responseJSON.message + "!");
        ...
        Feng.confirm("是否同意此社团财务申请? ", operation);
    }
};
...
$(function () {
    ...
    var table = new BSTable(Finance.id, "/finance/club_list", defaultColunms);
